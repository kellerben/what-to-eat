...
const vueapp = new Vue({
	...
	methods: {
		...
		setLatLon() {
			...
			if (typeof this.editShop.shop == 'undefined') {
				lunch.then((client) =>
					client.apis.Community.setCommunityInformation(
						{
		...
		getCommunityInformation() {
			lunch.then((client) =>
				client.apis.Community.getCommunityInformation({
					community: this.community,
				}).then(
					(result) => {
		...
		shops.forEach(function (elem) {
			...
			lunch
				...
				.then(
					...
					(reason) =>
						vueapp.warning('Could not fetch available shops. (' + reason + ')')
				);
		});
	}
	lunch
		.then((client) =>
			client.apis.Shop.getShops({ community: vueapp.community })
		)
		.then(
